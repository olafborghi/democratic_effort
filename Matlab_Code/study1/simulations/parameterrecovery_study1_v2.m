%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PARAMETER RECOVERY, STUDY 1
% Winning model: ms_three_k_one_beta_hyperbolic
%
% This script:
% 1. Loads the empirical Study 1 workspace
% 2. Checks that the loaded workspace contains the Study 1 27-trial schedule
...
%    /Users/marianavonmohr/Desktop/Effort_computationalmodels/study1/simulations
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sim_output_dir = '/Users/marianavonmohr/Desktop/Effort_computationalmodels/study1/simulations';
...
e       = 'PM';
modelID = 'ms_three_k_one_beta_hyperbolic';

bounds.beta = [0, 10];

%% Load fitted empirical Study 1 workspace

% IMPORTANT:
% If the newest workspace is not the true Study 1 workspace, hard-code the
% correct file below by replacing the automatic loading block.

files = dir(fullfile(workspace_dir, 'EM_fit_results_full_democraticeffortdata*.mat'));

if isempty(files)
    error('No Study 1 EM workspace found in %s', workspace_dir);
end
...
%% Check loaded workspace is Study 1
...
%% Use empirical Study 1 task schedule

% Study 1 should have 27 trials:
% 3 beneficiaries × 3 effort levels × 3 reward levels
% agents: 1 = all, 2 = rural, 3 = urban
...
for isub = 1:numel(s.(e).beh)
    ...
    if all(ismember({'agent','effort','reward','choice'}, Ttmp.Properties.VariableNames))
        ...
        if n_trials == 27 && isequal(agents_present(:), [1;2;3])
            schedule = Ttmp;
...
if isempty(schedule)
    error(['Could not find a valid Study 1 schedule. Expected 27 trials and agents 1, 2, 3. ', ...
           'Check that the loaded workspace is really Study 1.']);
end
...
fprintf('Using %d trials from empirical Study 1 schedule.\n', nTrials);
disp(tabulate(agent));

%% Define true parameter grid

% Following Cutler et al. style:
% K values across parameter space, beta as integers 0 to 10.
%
% For 3K1β:
% 4 K values for each of 3 K parameters and 11 beta values
% gives 4 × 4 × 4 × 11 = 704 simulated agents.
...
if any(beta_grid < bounds_full.lower(4)) || any(beta_grid > bounds_full.upper(4))
    error('beta_grid values fall outside fitted beta bounds.');
end

[K1, K2, K3, BETA] = ndgrid(k_grid, k_grid, k_grid, beta_grid);

true_params = [K1(:), K2(:), K3(:), BETA(:)];

param_names = {'k_agent1','k_agent2','k_agent3','beta'};
...
s_pr.PM.expname = 'ParameterRecoveryStudy1';
...
for isub = 1:nSimSubj
    ...
    k1   = true_params_noisy(isub,1);   % all
    k2   = true_params_noisy(isub,2);   % rural
    k3   = true_params_noisy(isub,3);   % urban
    beta = true_params_noisy(isub,4);   % shared beta
...
recovery_table.true_all_k   = true_params_noisy(:,1);
recovery_table.true_rural_k = true_params_noisy(:,2);
recovery_table.true_urban_k = true_params_noisy(:,3);
recovery_table.true_beta    = true_params_noisy(:,4);

recovery_table.recovered_all_k   = recovered_params(:,1);
recovery_table.recovered_rural_k = recovered_params(:,2);
recovery_table.recovered_urban_k = recovered_params(:,3);
recovery_table.recovered_beta    = recovered_params(:,4);
...
writetable(recovery_table, ...
    fullfile(sim_output_dir, 'parameter_recovery_study1_3K1B_true_vs_recovered.csv'));

writetable(recovery_summary, ...
    fullfile(sim_output_dir, 'parameter_recovery_study1_3K1B_summary.csv'));

writetable(confusion_table, ...
    fullfile(sim_output_dir, 'parameter_recovery_study1_3K1B_confusion_matrix.csv'));

save(fullfile(sim_output_dir, ['parameter_recovery_study1_3K1B_', date, '.mat']), ...
...
fig = figure('Color','w','Name','Study 1 parameter recovery');

labels_true = {'True all K','True rural K','True urban K','True beta'};
labels_rec  = {'Recovered all K','Recovered rural K','Recovered urban K','Recovered beta'};
...
exportgraphics(fig, ...
    fullfile(sim_output_dir, 'parameter_recovery_study1_3K1B_scatterplots.png'), ...
    'Resolution', 300);
