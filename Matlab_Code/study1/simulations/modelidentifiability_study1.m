%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MODEL IDENTIFIABILITY, STUDY 1
%
% Aim:
% Simulate data from each of the six Study 1 models, fit all six candidate
% models back to each simulated dataset, and check whether model comparison
% recovers the true generating model.
%
% Study 1 model space:
% 1. ms_three_k_one_beta
% 2. ms_three_k_one_beta_linear
% 3. ms_three_k_one_beta_hyperbolic
% 4. ms_three_k_three_beta
% 5. ms_three_k_three_beta_linear
% 6. ms_three_k_three_beta_hyperbolic
%
% Outputs saved to:
...
% Number of simulated datasets per generating model.
% Cutler et al. used 10. Start with 3 or 5 if runtime is too long.
nRounds = 10;

% Number of artificial agents per simulated dataset.
% Cutler et al. used 100.
nSubj = 100;

% Parameter ranges
bounds.beta = [0, 10];

% Use broad but valid ranges. These should match the fitted bounds.
% From your empirical fits, k bounds are [0, 2].
k_range = [0, 2];
beta_range = [0, 10];

%% Load empirical Study 1 workspace to get schedule
...
if isempty(schedule)
    error('Could not find valid Study 1 schedule. Expected 27 trials and agents 1, 2, 3.');
end
...
all_sim_struct = cell(nRounds, nModels);
...
for gen_m = 1:nModels
    ...
    for r = 1:nRounds
        ...
        for is = 1:nSubj
            ...
            % Draw native-space true parameters
            ...
            p_work = simulate_choice_probs_study1( ...
...
writetable(results_table, ...
    fullfile(sim_output_dir, 'model_identifiability_study1_results_long.csv'));

writetable(confusion_BIC_table, ...
    fullfile(sim_output_dir, 'model_identifiability_study1_confusion_BICint.csv'), ...
    'WriteRowNames', true);

writetable(confusion_XP_table, ...
    fullfile(sim_output_dir, 'model_identifiability_study1_confusion_XP.csv'), ...
    'WriteRowNames', true);

save(fullfile(sim_output_dir, ['model_identifiability_study1_', date, '.mat']), ...
...
function p_work = simulate_choice_probs_study1(effort, reward, agent, params, modelID)
