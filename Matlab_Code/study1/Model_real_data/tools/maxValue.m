% NOTE (MvM 2025): Adapted to handle Democratic Effort Task. 
% Works with multiple agents because it collapses across all trials 
% to compute global min/max effort and reward values.
...
% NOTE: rootfile.beh{1,1} is a struct array (one per trial).
% Using [ ... ] converts the comma-separated list of .effort or .reward
% fields into a numeric vector, so min/max operate correctly.
...
efforts = [rootfile.beh{1,1}.effort];
rewards = [rootfile.beh{1,1}.reward];

minEffort = min(efforts);
maxEffort = max(efforts);
