...
% - Recode agent column (VOTERURAL→1, VOTEURBAN→2)
...
dataFolder = '/Users/marianavonmohr/Desktop/Effort_computationalmodels/study2/Model_real_data/no_na';
...
for i = 1:N
    ...
    % Recode agent column for 2 agents (filter out any VOTEALL rows if present)
    if ismember('agent', T.Properties.VariableNames)
        T.agent = string(T.agent);

        % Keep only rural and urban trials (drop any "all" trials if they exist)
        keep = (T.agent == "VOTERURAL") | (T.agent == "VOTEURBAN");
        T = T(keep, :);

        % Recode
        T.agent(T.agent == "VOTERURAL") = "1";
        T.agent(T.agent == "VOTEURBAN") = "2";
...
save('/Users/marianavonmohr/Desktop/Effort_computationalmodels/study2/Model_real_data/full_democraticeffortdata_for_model.mat','s');
...
output_dir = '/Users/marianavonmohr/Desktop/Effort_computationalmodels/study2/PM_R_code/data/';
...
if M.dofit
    ...
    save(fullfile('/Users/marianavonmohr/Desktop/Effort_computationalmodels/study2/Model_real_data/workspaces', ...
...
%% Define model set to verify (2-k space)
e = 'PM';
modnames = {
   'ms_two_k_one_beta', 'ms_two_k_one_beta_linear', 'ms_two_k_one_beta_hyperbolic', ...
    'ms_two_k_two_beta', 'ms_two_k_two_beta_linear', 'ms_two_k_two_beta_hyperbolic'};
...
%%%%%%%%% this needs to be changed!!!! , the name of the winning model
%%%%%%%%% needs to be added fir best_fixed
best_fixed = 'ms_two_k_one_beta_hyperbolic';
...
T = params_fixed.all_table;   % expected columns include ID, k_agent1, k_agent2, beta OR beta_agent1/2
vn = T.Properties.VariableNames;

% ----- Reorder to a standard layout -----

want_shared_beta = {'ID','k_agent1','k_agent2','beta'};
want_two_beta    = {'ID','k_agent1','k_agent2','beta_agent1','beta_agent2'};

if all(ismember(want_shared_beta, vn))
    T = T(:, want_shared_beta);

elseif all(ismember(want_two_beta, vn))
    T = T(:, want_two_beta);

else
    error('Unexpected parameter structure. Columns found: %s', strjoin(vn, ', '));
end

% ----- Rename k parameters to meaningful labels -----

T.Properties.VariableNames{'k_agent1'} = 'rural_k';
T.Properties.VariableNames{'k_agent2'} = 'urban_k';

% ----- Rename beta parameters if agent-specific -----

vn = T.Properties.VariableNames;

if any(strcmp(vn,'beta_agent1'))
    T.Properties.VariableNames{'beta_agent1'} = 'beta_rural';
    T.Properties.VariableNames{'beta_agent2'} = 'beta_urban';
end
...
writetable(T, ...
    [output_dir, 'EM_fit_parameters_', include, '_', best_fixed, '_labeled.csv'], ...
    'WriteRowNames', false);
